...
public class HelloController {
    ...
    /**
     * 进行sentinel流控（直接编码的方式）
     * 这种方式，sentinel 对代码的侵入性太强了，写起来非常麻烦
     * 测试：http://localhost:8060/hello
     *
     * @return
     */
    ...
    public String hello() {
        ...
        try {
            // 1.sentinel针对资源进行限制的
            entry = SphU.entry(HELLO_RESOURCE_NAME);//定义资源
            // 被保护的业务逻辑
     ...
     * spring 的初始化方法
     *
     * 对hello和user这2个接口（资源）设置流控规则，QPS 为 2
     *
     */
    @PostConstruct  //
    ...
    @SentinelResource(value = USER_RESOURCE_NAME, fallback = "fallbackHandleForGetUser",
            ...
            /*blockHandlerClass = XXX.class,*/ blockHandler = "blockHandlerForGetUser")
